...
using namespace std;
constexpr int N=(1<<21)+100,_g=3,M1=1e9+7,M2=1e9+9,M=M2;
using ll=long long;
namespace fast_io{
...
template<int mod>struct NTT{
    ...
    vt mg(vt f,vt g){
        int r=bceil(f.size()+g.size()+1),i;
        for(i=0;i<r;++i)F[i]=G[i]=0;
        for(i=0;i<f.size();++i)F[i]=f[i];
        for(i=0;i<g.size();++i)G[i]=g[i];
        mul(F,G,r);
        vt h(F,F+f.size()+g.size()-1);
        return h;
    }
};
NTT<M>ntt;
...
const int G=115381398;
int T,n,m,K,Q,ed[N],b[N],d[N],f[3][1005][1005],w[12],rp[3][1005],ans;
int main(){
    ...
    for(cin>>T;T--;){
        ...
        ans=0;
        cin>>n>>m>>K;
        for(i=w[0]=1;i<=10;++i)w[i]=1ll*G*w[i-1]%M;
        for(i=0;i<3;++i)rp[i][0]=1,rp[i][1]=0;
        for(i=0;i<3;++i){
            k=K/3+(i<=K%3);
            for(j=0;j<3;++j)add(rp[j][1],k,w[i*j]);
        }
        for(i=0;i<3;++i)
            for(j=2;j<=n;++j)
                rp[i][j]=1ll*rp[i][1]*rp[i][j-1]%M;
        for(i=0;i<=n;++i)
            for(j=0;i+j<=n;++j){
                for(k=r=0;k<3;++k){
                    add(r,1ll*rp[k][i]*rp[(k+1)%3][j]%M*rp[(k+2)%3][n-i-j]%M);
                }
                add(ans,1ll*C(n,i)%M*C(n-i,j)%M*qp(r,m)%M);
            }
        ans=1ll*ans*qp(qp(3),n+m)%M;
        printf("%d\n",ans);
    }
    return 0;
}