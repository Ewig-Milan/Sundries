...
int main() {
    ...
    while(T--) {
        ...
        for(int i = 1, sum = 1; i <= n; i++) {
            ...
            if(v % p == 0) {
                f1[i] =(f1[i] + g[M[i + 1]]) % mod;
                f1[i] =(f1[i] + h[M[i + 1]]) % mod;
                f2[i] =(f2[i] + mod - h[M[i + 1]]) % mod;
            }
            sum =(sum + f1[i]) % mod;
            if(i >= t) {
                h[M[i - t + 1]] =(h[M[i - t + 1]] + f1[i - t]) % mod;
                g[M[i - t + 1]] =(g[M[i - t + 1]] + f2[i - t]) % mod;
            }
        }
        printf("%d\n",(f1[n] + f2[n]) % mod);
    }
